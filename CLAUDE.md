...
| 仪表盘 (`/dashboard`) | ⏳ 未实现 | 前端已调用，后端无 Controller/Service |
| 角色管理 (`/roles`) | ⏳ 未实现 | Entity 已有，无 Controller/Service/Mapper/DTO |
| GPA 计算 (`/gpa`) | ⏳ 未实现 | 前端已调用，后端无 Controller/Service |
| AI 分析 (`/ai-analysis`) | ⏳ 未实现 | Entity 已有，无 Controller/Service/Mapper/DTO |
| 系统设置 (`/settings`) | ⏳ 未实现 | Entity 已有，无 Controller/Service/Mapper/DTO |
| 导入导出 (`/data`) | ⏳ 未实现 | Entity 已有，无 Controller/Service/Mapper/DTO |
...
| 角色 (`/roles`) | ✅ UI 完成 | 完整 CRUD UI，已调 API 但后端未实现 |
| 个人中心 (`/profile`) | ✅ UI+API 完成 | 资料修改、密码修改、头像上传，已对接真实 API |
| 仪表盘 (`/dashboard`) | ✅ UI 完成 | 真实 UI + API 调用，后端 `/dashboard` 未实现 |
| 成绩/GPA (`/grades`) | ✅ UI 完成 | GPA 计算、趋势图、成绩列表，后端 `/gpa` 未实现 |
...
| 文件上传 (`/upload`) | ✅ 前后端已联调 |
| 角色管理 (`/roles`) | ⚠️ 前端完成，后端未实现 |
| 仪表盘 (`/dashboard`) | ⚠️ 前端完成，后端未实现 |
| 成绩/GPA (`/gpa`) | ⚠️ 前端完成，后端未实现 |
| AI 分析 (`/ai-analysis`) | ⏳ 前后端均未完成 |
| 系统设置 (`/settings`) | ⏳ 前后端均未完成 |
| 导入导出 (`/data`) | ⏳ 前后端均未完成 |
