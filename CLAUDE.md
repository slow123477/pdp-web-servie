...
| 仪表盘 (`/dashboard`) | ⚠️ 未实现 | 前端已调用，后端无 Controller/Service |
| 角色管理 (`/roles`) | ✅ 已完成 | 5 个接口（列表分页、详情、增删改），DTO/VO 模式 |
| GPA 计算 (`/gpa`) | ✅ 已完成 | 2 个接口（计算、趋势），复用 Course 数据 |
| AI 分析 (`/ai-analysis`) | ✅ 已完成 | 4 个接口（生成、列表、详情、删除），DTO/VO 模式 |
| 系统设置 (`/settings`) | ✅ 已完成 | 2 个接口（查询、修改） |
| 导入导出 (`/data`) | ⚠️ 未实现 | 前端已调用，后端无 Controller/Service |
...
| 系统设置 (`/settings`) | ✅ UI+API 完成 | GPA 标准设置、主题切换，已对接真实 API |
| 导入导出 (`/data`) | ✅ UI 完成 | 导入/导出/记录查询 UI，后端 `/data` 未实现 |
